...
Usage examples:
  ...
  python huntstand_exporter.py --cookies-file cookies.json
...
# ---- Logging ----
logging.basicConfig(level=os.getenv("HUNTSTAND_LOG_LEVEL", "INFO"),
...
# ---- Authentication ----
def create_session_from_cookies(sessionid: Optional[str], csrftoken: Optional[str]) -> Session:
...
def load_cookies_from_file(path: str) -> Tuple[Optional[str], Optional[str]]:
    """
...
def attempt_login(session: Session, username: str, password: str) -> bool:
    ...
    payload_common = {
        "csrfmiddlewaretoken": csrf_cookie or "",
        "password": password,
        "source": "web"
    }
    headers = {
...
# ---- Data Fetching ----
def gather_huntareas(session: Session, fallback_profile_id: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        logger.debug("About to check fallback: fallback_profile_id=%s, clubs length=%d", fallback_profile_id, len(clubs))
    ...
    logger.debug("Checking fallback condition: fallback_profile_id=%s, clubs length=%d", fallback_profile_id, len(clubs))
...
def fetch_members_for_area(session: Session, hunt_id: Any) -> Optional[Any]:
...
def write_membership_matrix(all_rows: List[Dict[str, Any]], out_path: str) -> None:
    ...
    emails = sorted(set((row.get("email") or "").lower().strip() for row in all_rows if row.get("email")))
    hunt_names = sorted(set(row.get("huntarea_name", "") for row in all_rows if row.get("huntarea_name")))
...
def write_per_hunt_csvs(all_rows: List[Dict[str, Any]]) -> None:
    ...
    for hid, items in grouped.items():
        safe_name = (items[0].get("huntarea_name") or f"Area-{hid}").replace("/", "_").replace("\\", "_") if items else f"Area-{hid}"
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--profile-id", help="Optional profile ID to fallback to /api/v1/huntarea/?profile_id=", default=ENV_PROFILE_ID)
    ...
    if not session.cookies.get("sessionid"):
        if args.no_login_fallback:
            logger.error("No sessionid cookie present and login fallback disabled. Provide cookies via env or --cookies-file.")
            return 2
        ...
        if ENV_USER and ENV_PASS:
            try:
                ...
                if not ok:
                    logger.warning("Login attempt did not produce session cookie. You may continue if API allows cookie-less access for some endpoints, otherwise provide cookies.")
            except Exception as e:
                logger.error("Login attempt raised exception: %s", e)
        else:
            logger.warning("No session cookie and no credentials configured (HUNTSTAND_USER/HUNTSTAND_PASS). Attempting to proceed; some endpoints may fail.")
    ...
    for club in normalized_clubs:
        ...
        # Invites
        for inv in invites:
            logger.debug("Processing invite: %s", json.dumps(inv, indent=2)[:500] + "..." if len(str(inv)) > 500 else json.dumps(inv, indent=2))
